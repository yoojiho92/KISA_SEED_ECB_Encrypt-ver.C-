...
public class KISA_SEED_ECB
{
    ...
    public static sbyte[] SEED_ECB_Encrypt(sbyte[] pbszUserKey, sbyte[] pbData, int offset, int length)
    {
        ...
        sbyte[] pbszPlainText = pbData;
        int nPlainTextLen = length;

        int nPlainTextPadding = (BLOCK_SIZE_SEED - (nPlainTextLen) % BLOCK_SIZE_SEED);
        sbyte[] newpbszPlainText = new sbyte[nPlainTextLen + nPlainTextPadding];
        Common.arraycopy(newpbszPlainText, pbszPlainText, nPlainTextLen);

        sbyte[] pbszCipherText = new sbyte[nPlainTextLen + nPlainTextPadding];
    ...
    /// <summary>
    /// @brief Data를 직접 Decrypt </summary>
    /// <param name="pbData"> : 복호화될 데이터 : 16 byte여야 함 </param>
    /// <param name="pdwRoundKey"> : SeedRoundKey()에 의해 만들어진 키 </param>
    ...
    public static sbyte[] SEED_ECB_Decrypt(sbyte[] pbszUserKey, sbyte[] pbData, int offset, int length)
    {
        ...
        sbyte[] newpbszCipherText = new sbyte[nCipherTextLen];
        Common.arraycopy(newpbszCipherText, pbszCipherText, nCipherTextLen);

        sbyte[] pbszPlainText = new sbyte[nCipherTextLen];
        ...
        if (SEED_ECB_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng) > 0)
        {
            ...
            sbyte[] result = new sbyte[message_length];
    ...
    public sealed class KISA_SEED_INFO
    {
        public int encrypt;
    ...
    public sealed class KISA_ENC_DEC
    {
        ...
        public static readonly KISA_ENC_DEC KISA_ENCRYPT = new KISA_ENC_DEC(_KISA_ENCRYPT);
        public static readonly KISA_ENC_DEC KISA_DECRYPT = new KISA_ENC_DEC(_KISA_DECRYPT);

    }

    public sealed class KISA_SEED_KEY
    {
    ...
    public static int SEED_ECB_init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, sbyte[] pbszUserKey)
    {
        ...
        K[30] = SS0[GetB0(T[0]) & 0x0ff] ^ SS1[GetB1(T[0]) & 0x0ff] ^ SS2[GetB2(T[0]) & 0x0ff] ^ SS3[GetB3(T[0]) & 0x0ff];
        K[31] = SS0[GetB0(T[1]) & 0x0ff] ^ SS1[GetB1(T[1]) & 0x0ff] ^ SS2[GetB2(T[1]) & 0x0ff] ^ SS3[GetB3(T[1]) & 0x0ff];
    ...
    public static int SEED_ECB_Close(KISA_SEED_INFO pInfo, int[] @out, int out_offset, int[] outLen)
    {
        ...
        if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt)
        {
            ...
            {
                Common.set_byte_for_int(pInfo.ecb_buffer, i, (sbyte)nPaddngLeng, ENDIAN);
            }
        ...
        else
        {
            ...
            if (nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED)
            {
                ...
                {
                    Common.set_byte_for_int(@out, out_offset - i, (sbyte)0x00, ENDIAN);
                }
    ...
    /// <summary>
    ///********************** Constants for Key schedule ************************* </summary>
    //  KC0 = golden ratio; KCi = ROTL(KCi-1, 1)
    private const int KC0 = unchecked((int)0x9e3779b9);
    private const int KC1 = 0x3c6ef373;
    private const int KC2 = 0x78dde6e6;
    private const int KC3 = unchecked((int)0xf1bbcdcc);
    private const int KC4 = unchecked((int)0xe3779b99);
    private const int KC5 = unchecked((int)0xc6ef3733);
    private const int KC6 = unchecked((int)0x8dde6e67);
    private const int KC7 = 0x1bbcdccf;
    private const int KC8 = 0x3779b99e;
    private const int KC9 = 0x6ef3733c;
    private const int KC10 = unchecked((int)0xdde6e678);
    private const int KC11 = unchecked((int)0xbbcdccf1);
    private const int KC12 = 0x779b99e3;
    private const int KC13 = unchecked((int)0xef3733c6);
    private const int KC14 = unchecked((int)0xde6e678d);
    private const int KC15 = unchecked((int)0xbcdccf1b);

    /// <summary>
    ///************************ Macros for Key schedule ************************** </summary>
    ...
    /// <summary>
    /// @brief UserKey를 이용하여 RoundKey 생성 </summary>
    /// <param name="pbUserKey"> : RoundKey를 생성할 기본 키 </param>
    /// <param name="pdwRoundKey"> : pbUserKey로 생성되는 결과 pdwRoundKey (32*4 byte) </param>
    public static void SeedRoundKey(int[] pdwRoundKey, sbyte[] pbUserKey)
    {
    ...
    public class Common
    {
        ...
        public static void arraycopy(sbyte[] dst, sbyte[] src, int length)
        {
        ...
        public static void arraycopy_offset(sbyte[] dst, int dst_offset, sbyte[] src, int src_offset, int length)
        {
            for (int i = 0; i < length; i++)
            {
        ...
        public static void arrayinit(sbyte[] dst, sbyte value, int length)
        {
            for (int i = 0; i < length; i++)
            {
                dst[i] = value;
        ...
        public static void arrayinit_offset(sbyte[] dst, int dst_offset, sbyte value, int length)
        {
            for (int i = 0; i < length; i++)
            {
        ...
        public static void memcpy(int[] dst, sbyte[] src, int length, int ENDIAN)
        {
            int iLen = length / 4;
        ...
        public static void set_byte_for_int(int[] dst, int b_offset, sbyte value, int ENDIAN)
        {
        ...
        public static sbyte get_byte_for_int(int[] src, int b_offset, int ENDIAN)
        {
            if (ENDIAN == BIG_ENDIAN)
            {
                int shift_value = (3 - b_offset % 4) * 8;
        ...
        public static sbyte[] get_bytes_for_ints(int[] src, int offset, int ENDIAN)
        {
            int iLen = src.Length - offset;
            sbyte[] result = new sbyte[(iLen) * 4];
            for (int i = 0; i < iLen; i++)
            {
        ...
        public static void byte_to_int(int[] dst, int dst_offset, sbyte[] src, int src_offset, int ENDIAN)
        {
            if (ENDIAN == BIG_ENDIAN)
            {
        ...
        public static int byte_to_int(sbyte[] src, int src_offset, int ENDIAN)
        {
            if (ENDIAN == BIG_ENDIAN)
            {
        ...
        public static int byte_to_int_big_endian(sbyte[] src, int src_offset)
        {
            return ((0x0ff & src[src_offset]) << 24) | ((0x0ff & src[src_offset + 1]) << 16) | ((0x0ff & src[src_offset + 2]) << 8) | ((0x0ff & src[src_offset + 3]));
        }

        public static void int_to_byte(sbyte[] dst, int dst_offset, int[] src, int src_offset, int ENDIAN)
        {
            int_to_byte_unit(dst, dst_offset, src[src_offset], ENDIAN);
        }

        public static void int_to_byte_unit(sbyte[] dst, int dst_offset, int src, int ENDIAN)
        {
            if (ENDIAN == BIG_ENDIAN)
            {
                dst[dst_offset] = unchecked((sbyte)((src >> 24) & 0x0ff));
                dst[dst_offset + 1] = unchecked((sbyte)((src >> 16) & 0x0ff));
                dst[dst_offset + 2] = unchecked((sbyte)((src >> 8) & 0x0ff));
                dst[dst_offset + 3] = unchecked((sbyte)((src) & 0x0ff));
            }
            else
            {
                dst[dst_offset] = unchecked((sbyte)((src) & 0x0ff));
                dst[dst_offset + 1] = unchecked((sbyte)((src >> 8) & 0x0ff));
                dst[dst_offset + 2] = unchecked((sbyte)((src >> 16) & 0x0ff));
                dst[dst_offset + 3] = unchecked((sbyte)((src >> 24) & 0x0ff));
            }

        }
        public static void int_to_byte_unit_big_endian(sbyte[] dst, int dst_offset, int src)
        {
            dst[dst_offset] = unchecked((sbyte)((src >> 24) & 0x0ff));
            dst[dst_offset + 1] = unchecked((sbyte)((src >> 16) & 0x0ff));
            dst[dst_offset + 2] = unchecked((sbyte)((src >> 8) & 0x0ff));
    ...
    public static sbyte[] str2hex(string strData)
    {
        string resultHex = string.Empty;
        byte[] arr_byteStr = Encoding.Default.GetBytes(strData);
        return (sbyte[])(Array)arr_byteStr;
    }

    public static sbyte[] Str_Encrypt(String str)
    {
        String result = "";
        // User secret key
        //sbyte[] pbUserKey = new sbyte[] { unchecked((sbyte)0x88), unchecked((sbyte)0xE3), (sbyte)0x4F, unchecked((sbyte)0x8F), (sbyte)0x08, (sbyte)0x17, (sbyte)0x79, unchecked((sbyte)0xF1), unchecked((sbyte)0xE9), unchecked((sbyte)0xF3), unchecked((sbyte)0x94), (sbyte)0x37, (sbyte)0x0A, unchecked((sbyte)0xD4), (sbyte)0x05, unchecked((sbyte)0x89) };
        sbyte[] pbUserKey = str2hex("AT_QTS_2019_asdf");
        sbyte[] pdData3 = str2hex(str); // 암호화 할 데이터
        sbyte[] pbCipher = new sbyte[50];

        pbCipher = SEED_ECB_Encrypt(pbUserKey, pdData3, 0, pdData3.Length);
        return pbCipher;
    }

    public static String str_Decrypt(sbyte[] encrpyt)
    {
        String result = "";
        // User secret key
        //sbyte[] pbUserKey = new sbyte[] { unchecked((sbyte)0x88), unchecked((sbyte)0xE3), (sbyte)0x4F, unchecked((sbyte)0x8F), (sbyte)0x08, (sbyte)0x17, (sbyte)0x79, unchecked((sbyte)0xF1), unchecked((sbyte)0xE9), unchecked((sbyte)0xF3), unchecked((sbyte)0x94), (sbyte)0x37, (sbyte)0x0A, unchecked((sbyte)0xD4), (sbyte)0x05, unchecked((sbyte)0x89) };
        sbyte[] pbUserKey = str2hex("AT_QTS_2019_asdf");
        sbyte[] pbCipher = encrpyt;
        sbyte[] pbPlain = new sbyte[16];

        pbPlain = SEED_ECB_Decrypt(pbUserKey, pbCipher, 0, pbCipher.Length);

        /*
        Console.Write("Plaintext(dec)\t: ");
        for (int j = 0; j < pbPlain.Length; j++)
        {
            Console.Write((0xff & pbPlain[j]).ToString("x") + " ");
        }
        Console.Write("\n\n");
        */    
        result = Encoding.Default.GetString((byte[])(Array)pbPlain);
        return result;
    }
    ...
    public static void Main(string[] args)
    {
        ...
        sbyte[] encrpyt = Str_Encrypt("01092436401");    // 암호화

        //암호화한 값 출력
        for (int j = 0; j < encrpyt.Length; j++)
        {
            Console.Write((0xff & encrpyt[j]).ToString("x") + " ");
        }

        Console.Write("\n" + str_Decrypt(encrpyt));    //복호화 하여 출력
    }
}
